<template>
  <div class="login">
    <div class="form">
      ...
      plain type="primary"
      class="btn"
      @click="changePwsd" :loading="loading"
        >提 交</van-button
      >

    </div>
    <div class="nobox">
      <span class="nouser">不改了？</span><span class="register" @click="login">去登录</span>
    </div>
  </div>
</template>

<script>
import {change_password} from "@/service/index";
...
export default {
  ...
  methods: {
      ...
      async changePwsd(){
        if(!this.form.old_password || this.form.old_password.length<6){
            this.setMsg('请输入正确的密码！','red');
            return false;
        }
        ...
        if(this.form.new_password1 !== this.form.new_password){
